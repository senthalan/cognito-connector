 ...
 * under the License.
 */
package org.wso2.carbon.identity.application.authenticator.oidc.custom;
...
import org.wso2.carbon.identity.application.authenticator.oidc.custom.internal.CustomOpenIDConnectAuthenticatorDataHolder;
...
public class CustomOpenIDConnectAuthenticator extends AbstractApplicationAuthenticator
    ...
    public boolean canHandle(HttpServletRequest request) {
        ...
        if (log.isTraceEnabled()) {
            log.trace("Inside CustomOpenIDConnectAuthenticator.canHandle()");
        }

        // Check commonauth got an OIDC response
        if (request.getParameter(CustomOIDCAuthenticatorConstants.OAUTH2_GRANT_TYPE_CODE) != null
                && request.getParameter(CustomOIDCAuthenticatorConstants.OAUTH2_PARAM_STATE) != null
                && CustomOIDCAuthenticatorConstants.LOGIN_TYPE.equals(getLoginType(request))) {
            return true;
        } else if (request.getParameter(CustomOIDCAuthenticatorConstants.OAUTH2_PARAM_STATE) != null &&
                request.getParameter(CustomOIDCAuthenticatorConstants.OAUTH2_ERROR) != null) {
    ...
    protected String getTokenEndpoint(Map<String, String> authenticatorProperties) {
        ...
        return authenticatorProperties.get(CustomOIDCAuthenticatorConstants.OAUTH2_TOKEN_URL);
    }
    ...
    protected String getScope(String scope, Map<String, String> authenticatorProperties) {
        if (StringUtils.isBlank(scope)) {
            scope = CustomOIDCAuthenticatorConstants.OAUTH_OIDC_SCOPE;
        }
        return scope;
    }
    ...
    protected String getAuthenticateUser(AuthenticationContext context, Map<String, Object> oidcClaims,
        ...
        return (String) oidcClaims.get(CustomOIDCAuthenticatorConstants.Claim.SUB);
    }
    ...
    protected Map<ClaimMapping, String> getSubjectAttributes(OAuthClientResponse token,
        ...
        try {
            String accessToken = token.getParam(CustomOIDCAuthenticatorConstants.ACCESS_TOKEN);
        ...
        try {
            ...
            if (authenticatorProperties != null) {
                ...
                if (StringUtils.isNotBlank(queryString)) {
                    ...
                    context.setProperty(CustomOIDCAuthenticatorConstants.OIDC_QUERY_PARAM_MAP_PROPERTY_KEY, paramValueMap);
                }
                ...
                if (StringUtils.isNotBlank(queryString) && queryString.toLowerCase().contains("scope=") && queryString
                    ...
                    authzRequest = OAuthClientRequest.authorizationLocation(authorizationEP).setClientId(clientId)
                            .setResponseType(CustomOIDCAuthenticatorConstants.OAUTH2_GRANT_TYPE_CODE).setState(state)
                            .buildQueryMessage();
                    ...
                    authzRequest = OAuthClientRequest.authorizationLocation(authorizationEP).setClientId(clientId)
                            .setResponseType(CustomOIDCAuthenticatorConstants.OAUTH2_GRANT_TYPE_CODE)
                            .setScope(CustomOIDCAuthenticatorConstants.OAUTH_OIDC_SCOPE).setState(state).buildQueryMessage();

                } else {
                    authzRequest = OAuthClientRequest.authorizationLocation(authorizationEP).setClientId(clientId)
                            .setRedirectURI(callbackurl)
                            .setResponseType(CustomOIDCAuthenticatorConstants.OAUTH2_GRANT_TYPE_CODE).setScope(scope)
    ...
    private String getStateParameter(AuthenticationContext context, Map<String, String> authenticatorProperties) {
        String state = context.getContextIdentifier() + "," + CustomOIDCAuthenticatorConstants.LOGIN_TYPE;
    ...
    private String getOIDCAuthzEndpoint(Map<String, String> authenticatorProperties) {
        ...
        if (StringUtils.isBlank(authorizationEP)) {
            authorizationEP = authenticatorProperties.get(CustomOIDCAuthenticatorConstants.OAUTH2_AUTHZ_URL);
        }
    ...
    protected void processAuthenticationResponse(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            String idToken = oAuthResponse.getParam(CustomOIDCAuthenticatorConstants.ID_TOKEN);
            Map<String, String> authenticatorProperties = context.getAuthenticatorProperties();
            if (StringUtils.isBlank(idToken) && requiredIDToken(authenticatorProperties)) {
                throw new AuthenticationFailedException("Id token is required and is missing in OIDC response from "
                        ...
                        authenticatorProperties.get(CustomOIDCAuthenticatorConstants.CLIENT_ID));
            }
    ...
    protected OAuthClientRequest getAccessTokenRequest(AuthenticationContext context, OAuthAuthzResponse
        ...
        boolean isHTTPBasicAuth = Boolean.parseBoolean(authenticatorProperties.get(CustomOIDCAuthenticatorConstants
    ...
    public String getContextIdentifier(HttpServletRequest request) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Inside CustomOpenIDConnectAuthenticator.getContextIdentifier()");
        }
    ...
    private String getUserIdClaimUriInOIDCDialect(String userIdClaimInLocalDialect, String spTenantDomain)
        ...
        List<ExternalClaim> externalClaims = CustomOpenIDConnectAuthenticatorDataHolder.getInstance()
    ...
    private String getCallbackUrlFromInitialRequestParamMap(AuthenticationContext context) {
        ...
        @SuppressWarnings({"unchecked"}) Map<String, String> paramValueMap = (Map<String, String>) context
                .getProperty(CustomOIDCAuthenticatorConstants.OIDC_QUERY_PARAM_MAP_PROPERTY_KEY);

        if (MapUtils.isNotEmpty(paramValueMap) && paramValueMap.containsKey(CustomOIDCAuthenticatorConstants.REDIRECT_URI)) {
            return paramValueMap.get(CustomOIDCAuthenticatorConstants.REDIRECT_URI);
        }
